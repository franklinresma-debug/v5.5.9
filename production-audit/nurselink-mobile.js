...
/**
 * NurseLink Global Mobile + Auth UI Layer v5.5.3
 *
 * HOTFIX:
 * v1.2.0 could detach the registration form before its replacement
 * layout was inserted. v5.5.3 inserts the new shell first, then moves
...
(() => {
  ...
  /* =========================================================
     NurseLink v5.5.3 — normal-browser stale session recovery
     ========================================================= */
  ...
  window.fetch = async function nurseLinkFetch(input, init = {}) {
    ...
    if (
      ...
      if (loginResponse.ok) {
        ...
        /*
         * v5.5.3: support tightly-scoped local post-login returns for Partner
        ...
        if (
          ...
          (
            ...
            returnPath.startsWith('/dashboard?admin_invite=')
          )
        ) {
          setTimeout(() => {
    ...
    if (
      ...
      /*
       * v5.5.3: privileged identity is irrelevant on public auth screens.
  ...
  /* NurseLink v5.5.3 — shared authenticated JSON API request helper */
  ...
  async function enhanceAdministratorInvitationAcceptance() {
    if (location.pathname.replace(/\/+$/, '') !== '/dashboard') return;

    const token = new URLSearchParams(location.search).get('admin_invite') || '';
    if (token.length < 32 || document.documentElement.dataset.nlAdminInviteHandled === '1') return;

    document.documentElement.dataset.nlAdminInviteHandled = '1';

    const banner = document.createElement('section');
    banner.className = 'nurselink-admin-invite-banner';
    banner.setAttribute('role', 'status');

    const renderInviteBanner = (title, message, href = '') => {
      ...
      if (href) {
        ...
        link.href = href;
        link.textContent = 'Open Administrator Login';
        banner.append(link);
      }
    };

    renderInviteBanner('Administrator invitation', 'Confirming your verified NurseLink account…');
    (document.querySelector('main') || document.body).prepend(banner);

    try {
      const result = await nurseLinkJsonRequest(
        NURSELINK_API_ORIGIN,
        '/api/nurselink/admin/management/invitations/accept',
        {
          method: 'POST',
      ...
      clean.searchParams.delete('admin_invite');
      history.replaceState(null, '', clean.pathname + (clean.search || '') + (clean.hash || ''));

      renderInviteBanner(
        'Administrator access activated',
        String(result?.message || 'Your Administrator invitation has been accepted.'),
        '/nurselink-admin-login.html'
      );
    } catch (error) {
      renderInviteBanner(
        'Administrator invitation needs attention',
        String(error?.message || 'Unable to accept this Administrator invitation.')
      );
    }
  }
    ...
    if (title) {
      ...
      if (!sourceContainer.querySelector('.nurselink-auth-form-subtitle')) {
        ...
        subtitle.textContent = type === 'register'
          ? 'Create your secure account first. Then upload your documents and NurseLink will pre-fill as much of your membership application as possible.'
  ...
  /* =========================================================
     NurseLink Professional Onboarding — v5.5.3
    ...
    card.innerHTML = `
      <div class="nurselink-command-main">
        ...
        <div class="nurselink-command-actions">
          <a class="primary-button" href="/smart-registration?smartstep=1">Continue application →</a>
      ...
      <div class="nurselink-command-progress-wrap">
        ...
        <div class="nurselink-command-facts">
          ...
          <div>
            ...
            <strong>Smart Registration</strong>
          </div>
          <div>
  ...
  /* =========================================================
     NurseLink Profile Photo — v5.5.3
  ...
  /* =========================================================
     NurseLink v5.5.3 — Qualification Readiness
  ...
  /* =========================================================
     NurseLink v5.5.3 — Approved Member Dashboard
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — Career Matching + Learning / CPD
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — Reviewer / Admin Verification Center
     ========================================================= */
  ...
  function renderDigitalMembershipCard(data) {
    ...
    card.dataset.nurselinkDigitalId = 'v5.5.3';
  ...
  async function enhanceMembershipIdentity(page) {
    ...
    if (route === 'application-status') {
      ...
      status.innerHTML = `
        ...
        ${Array.isArray(membership.status_history) && membership.status_history.length ? `
          <div class="nurselink-membership-timeline" aria-label="Membership application timeline">
            <div class="nurselink-membership-timeline-head">
              <span>APPLICATION TIMELINE</span>
              <strong>${membership.status_history.length} update${membership.status_history.length === 1 ? '' : 's'}</strong>
            </div>
            <ol>
              ${membership.status_history.slice(-10).reverse().map(item => {
                ...
                return `<li>
                  ...
                  <div><strong>${nlV200Escape(membershipStatusLabel(item?.to_status || 'submitted'))}</strong>${whenText ? `<time>${nlV200Escape(whenText)}</time>` : ''}${item?.reason ? `<p>${nlV200Escape(item.reason)}</p>` : ''}</div>
                </li>`;
              }).join('')}
            </ol>
          </div>
  ...
  function openNotificationDrawer(button) {
    ...
    /*
     * v5.5.3 — CACHE-FIRST NOTIFICATION DRAWER
     *
  ...
  function membershipEvidenceHtml(data) {
    const profile = data?.profile || null;
    const documents = Array.isArray(data?.documents) ? data.documents : [];
    const quality = data?.extraction_summary || {};
    const value = item => nlV200Escape(item || 'Not provided');
    const fullName = profile
      ? [profile.first_name, profile.middle_name, profile.last_name].filter(Boolean).join(' ')
      : '';

    return `
      <div class="nurselink-membership-evidence-summary">
        <div class="nurselink-smart561-quality" data-review-required="${quality.review_required ? 'true' : 'false'}">
          <div>
            <strong>${quality.review_required ? 'Extraction needs reviewer attention' : 'No extraction conflicts detected'}</strong>
            <small>${Number(quality.fields_detected || 0)} fields detected · ${Number(quality.applicant_confirmed_fields || 0)} applicant-confirmed · ${Number(quality.current_documents || 0)} current documents</small>
          </div>
          <div class="nurselink-smart561-quality-counts">
            <span><b>${Array.isArray(quality.conflicts) ? quality.conflicts.length : 0}</b> conflicts</span>
            <span><b>${Array.isArray(quality.low_confidence_fields) ? quality.low_confidence_fields.length : 0}</b> low confidence</span>
            <span><b>${Number(quality.documents_needing_manual_input || 0)}</b> manual input</span>
          </div>
          ${Array.isArray(quality.conflicts) && quality.conflicts.length ? `
            <div class="nurselink-smart561-review-issues">
              <strong>Conflicting document values</strong>
              ${quality.conflicts.map(issue => `<span><b>${value(String(issue.field || '').replace(/_/g, ' '))}</b>${(issue.candidates || []).map(candidate => `${value(candidate.value)} (${Math.round(Number(candidate.confidence || 0) * 100)}%, ${value(candidate.source_name)})`).join(' · ')}</span>`).join('')}
            </div>
          ` : ''}
          ${Array.isArray(quality.low_confidence_fields) && quality.low_confidence_fields.length ? `
            <div class="nurselink-smart561-review-issues">
              <strong>Low-confidence values</strong>
              ${quality.low_confidence_fields.map(issue => `<span><b>${value(String(issue.field || '').replace(/_/g, ' '))}</b>${value(issue.candidate?.value)} · ${Math.round(Number(issue.candidate?.confidence || 0) * 100)}%</span>`).join('')}
            </div>
          ` : ''}
        </div>

        <strong>Applicant-confirmed profile</strong>
        ${profile ? `
          <div class="nurselink-membership-evidence-grid">
            <span><small>Name</small>${value(fullName)}</span>
            <span><small>Professional title</small>${value(profile.professional_title)}</span>
            <span><small>Current position</small>${value(profile.current_position)}</span>
            <span><small>Employer</small>${value(profile.current_employer)}</span>
            <span><small>License / registration</small>${value(profile.primary_license_number)}</span>
            <span><small>License expiry</small>${value(profile.primary_license_expiry)}</span>
            <span><small>Nursing education</small>${value(profile.highest_nursing_education)}</span>
            <span><small>Experience</small>${profile.years_experience === null || profile.years_experience === undefined ? 'Not provided' : `${Number(profile.years_experience)} years`}</span>
          </div>
        ` : '<p>No Smart Registration profile is available.</p>'}

        <strong>Supporting evidence (${documents.length})</strong>
        <div class="nurselink-membership-evidence-docs">
          ${documents.length ? documents.map(doc => `
            <div>
              <span><b>${value(doc.name)}</b><small>${value(doc.document_type)} · ${value(doc.extraction_status)}${doc.is_current === false ? ' · Superseded' : ''}</small></span>
              <a href="${nlV200Escape(`https://api.amsertech.com${doc.download_url}`)}" target="_blank" rel="noopener">Open evidence</a>
            </div>
          `).join('') : '<p>No supporting documents were attached.</p>'}
        </div>

        <p class="nurselink-membership-evidence-boundary">Extraction assists data entry only. Reviewers must independently verify credential and identity evidence.</p>
      </div>`;
  }

  async function toggleMembershipEvidence(record, card, button) {
    const panel = card.querySelector('.nurselink-membership-evidence');
    if (!panel) return;
    ...
    panel.innerHTML = '<p>Loading applicant profile and supporting documents…</p>';

    try {
      const payload = await reviewerRequest(`/membership-applications/${record.id}/evidence`);
      panel.innerHTML = membershipEvidenceHtml(payload?.data || null);
      button.textContent = 'Hide Evidence';
    } catch (error) {
      panel.innerHTML = `<div class="form-error">${nlV200Escape(error.message)}</div>`;
      button.textContent = 'Retry Evidence';
    } finally {
      button.disabled = false;
    }
  }
  ...
  function renderMembershipQueue(container) {
    ...
    rows.forEach(record => {
      ...
      card.innerHTML = `
        ...
        <div class="nurselink-membership-evidence" hidden></div>

        <div class="nurselink-review-card-actions">
          <button type="button" class="secondary-button" data-action="inspect-evidence">Inspect Evidence</button>
      ...
      card.querySelector('[data-action="inspect-evidence"]')
        ?.addEventListener('click', event =>
          toggleMembershipEvidence(record, card, event.currentTarget)
        );
      ...
      list.innerHTML = `
        <div class="nurselink-review-empty">
          ...
          <small>
            ...
            access NurseLink after v5.5.3 is installed.
          </small>
        </div>
      `;
    }
  ...
  /* =========================================================
     NurseLink v5.5.3 — Shareable Digital Nurse Profile
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — stale browser cache defense
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — Partner Portal launcher
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — Candidate Messaging + Interviews
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — Institutional Analytics launcher
     ========================================================= */
  ...
  /* NurseLink v5.5.3 — Production UAT launcher */

  /*
   * NurseLink v5.5.3 — build-persistent release marker.
  ...
  function enhanceProductionUatLauncher(page) {
    ...
    card.dataset.nurselinkRelease = '5.5.8';
  ...
  function enhanceEnterpriseLauncher(page) {
    ...
    card.dataset.nurselinkEnterprise = 'v5.5.3';
  ...
  function enhanceBenefitsLauncher(page) {
    ...
    card.dataset.nurselinkBenefits = 'v5.5.3';
  ...
  function enhanceEngagementHubLauncher(page) {
    ...
    card.dataset.nurselinkEngagementHub = 'v5.5.3';
  ...
  async function enhanceMentoringLauncher(page) {
    ...
    card.dataset.nurselinkMentoring = 'v5.5.3';
  ...
  function enhanceChaptersLauncher(page) {
    ...
    card.dataset.nurselinkChapters = 'v5.5.3';
  ...
  function enhanceEventsProgramsLauncher(page) {
    ...
    card.dataset.nurselinkEventsPrograms = 'v5.5.3';
  ...
  /* =========================================================
     NurseLink v5.5.3 — Server-confirmed Super Administrator identity
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — System Access vs Membership Identity Clarity
     ========================================================= */
  ...
  /* =========================================================
     NurseLink v5.5.3 — Two-Portal Consolidation
  ...
  /* =========================================================
     NurseLink v5.5.7 — Smart Membership Registration
     Four-step, document-first applicant flow.
     Extracted values are suggestions only and remain applicant-
     confirmed until NurseLink reviewer verification.
     ========================================================= */

  const SMART_REGISTRATION_API = 'https://api.amsertech.com/api/smart-registration';
  const smartRegistration557State = {
    loaded: false,
    loading: null,
    data: null,
    notice: '',
  ...
  const SMART_REGISTRATION_STEPS = [
    { number: 1, title: 'Upload Documents', short: 'Upload' },
    { number: 2, title: 'Personal Information', short: 'Personal' },
    { number: 3, title: 'Professional Details', short: 'Professional' },
    { number: 4, title: 'Review & Submit', short: 'Review' }
  ];

  function smartEsc(value) {
    return String(value ?? '')
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  function smartStep() {
    const params = new URLSearchParams(location.search);
    const explicit = Number(params.get('smartstep') || 0);
    if (Number.isFinite(explicit) && explicit >= 1 && explicit <= 4) return explicit;

    // Preserve older cumulative links while routing them into the new four-step flow.
    const legacy = Number(params.get('nlstep') || 0);
    if (legacy === 3) return 3;
    if (legacy === 5) return 1;
    if (legacy === 6) return 4;
    return 1;
  }

  function smartSetStep(step) {
    const next = Math.max(1, Math.min(4, Number(step) || 1));
    const url = new URL(location.href);
    url.searchParams.delete('nlstep');
    url.searchParams.set('smartstep', String(next));
    history.pushState(null, '', url.pathname + url.search + url.hash);
    const page = document.querySelector('.page') || document.querySelector('main');
    renderSmartRegistration557(page);
    page?.querySelector('.nurselink-smart557')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  async function smartRegistrationRequest(path = '', options = {}) {
    return nurseLinkJsonRequest(SMART_REGISTRATION_API, path, options);
  }

  async function loadSmartRegistration557(force = false) {
    if (smartRegistration557State.loaded && !force) return smartRegistration557State.data;
    if (smartRegistration557State.loading && !force) return smartRegistration557State.loading;

    smartRegistration557State.loading = smartRegistrationRequest('')
      .then(payload => {
        smartRegistration557State.data = payload?.data || {};
        smartRegistration557State.loaded = true;
        smartRegistration557State.loading = null;
        return smartRegistration557State.data;
      })
      .catch(error => {
        smartRegistration557State.loading = null;
        throw error;
    ...
    return smartRegistration557State.loading;
  }

  function smartSuggestion(field, profile, suggestions) {
    const saved = profile?.[field];
    if (saved !== null && saved !== undefined && String(saved).trim() !== '') {
      return { value: saved, detected: false, source: '', confidence: 1 };
    }

    const suggestion = suggestions?.[field];
    if (!suggestion || suggestion.value === null || suggestion.value === undefined) {
      return { value: '', detected: false, source: '', confidence: 0 };
    }

    return {
      value: suggestion.value,
      detected: true,
      source: suggestion.source_name || 'uploaded document',
      confidence: Number(suggestion.confidence || 0)
    };
  }

  function smartFieldMeta(field, profile, suggestions, fieldStatuses = {}) {
    const suggestion = suggestions?.[field] || null;
    const status = fieldStatuses?.[field]?.status
      || (suggestion ? 'extracted' : 'missing');
    const labels = {
      extracted: 'Extracted',
    ...
    const source = suggestion?.source_name || 'uploaded document';
    const alternatives = Array.isArray(suggestion?.alternatives) ? suggestion.alternatives : [];

    return `
      <span class="nurselink-smart561-field-meta" data-status="${smartEsc(status)}">
        <b>${smartEsc(labels[status] || status)}</b>
        ${suggestion ? `<small>${smartEsc(source)}${confidence ? ` · ${confidence}% confidence` : ''}</small>` : ''}
      </span>
      ${suggestion?.conflict ? `
        <span class="nurselink-smart561-conflict">
          <strong>Different values were found. Choose the correct one:</strong>
          <span>${alternatives.map(item => `
            <button type="button" data-smart-use-field="${smartEsc(field)}" data-smart-use-value="${smartEsc(item.value)}">
              ${smartEsc(item.value)} <small>${smartEsc(item.source_name || 'document')} · ${Math.round(Number(item.confidence || 0) * 100)}%</small>
            </button>
          `).join('')}</span>
        </span>
      ` : ''}`;
  ...
  function smartSourcesFor(fields, suggestions, values = {}) {
    const result = {};
    fields.forEach(field => {
      const suggestion = suggestions?.[field];
      const submitted = values?.[field];
      const sameAsDetected = suggestion?.source_document_id
        && String(submitted ?? '').trim() === String(suggestion.value ?? '').trim();

      if (sameAsDetected) {
        result[field] = {
          source: 'document',
          document_id: suggestion.source_document_id,
          document_name: suggestion.source_name || '',
          confidence: Number(suggestion.confidence || 0)
        };
      } else {
        result[field] = { source: 'applicant' };
      }
    });
    return result;
  }

  function smartMissingForStep(missing, step) {
    ...
    return `
      ...
      <div class="nurselink-smart557-stepper" aria-label="Registration progress">
        ${SMART_REGISTRATION_STEPS.map(item => `
          <button type="button" class="${item.number === step ? 'active' : item.number < step ? 'done' : ''}" data-smart-step="${item.number}">
            <span>${item.number < step ? '✓' : item.number}</span>
            <strong>${smartEsc(item.title)}</strong>
          </button>
        `).join('')}
      </div>
      <div class="nurselink-smart557-progress" aria-label="${completion}% registration completion"><i style="width:${completion}%"></i></div>
    `;
  }
  ...
  function smartUploadStep(data) {
    const documents = Array.isArray(data?.documents) ? data.documents : [];
    const capabilities = data?.extraction_capabilities || {};
    const missing = smartMissingForStep(data?.missing, 1);

    return `
      <section class="nurselink-smart557-card nurselink-smart557-upload-card">
        <div class="nurselink-smart557-card-heading centered">
          <span class="nurselink-smart557-icon">⇧</span>
          <h1>Start by Uploading Your Documents</h1>
        ...
        ${documents.length ? `
          <div class="nurselink-smart557-documents">
            <div class="nurselink-smart557-subhead">
            ...
            ${documents.map(doc => {
              ...
              return `
                <div class="nurselink-smart557-document">
                  ...
                  <span class="copy"><strong>${smartEsc(doc.name)}</strong><small>${smartEsc(doc.document_type || 'document')} · v${smartEsc(doc.version || 1)} · ${smartEsc(status)}</small></span>
                  <span class="nurselink-smart558-document-actions">
                    <button type="button" class="replace" data-smart-replace-document="${doc.id}" aria-label="Replace ${smartEsc(doc.name)}">Replace</button>
                    <button type="button" class="remove" data-smart-remove-document="${doc.id}" aria-label="Remove ${smartEsc(doc.name)}">×</button>
                  </span>
                </div>
              `;
            }).join('')}
        ...
        <div class="nurselink-smart557-privacy">
          <span>▣</span>
          <p><strong>Your documents are stored in private application storage and are never published publicly.</strong><br>Extracted values help fill the application, but you will review and confirm everything before submission.</p>
        </div>

        ${(!capabilities.image_ocr || !capabilities.pdf_text) ? `
          <div class="nurselink-smart557-capability-note">
            <strong>Smart extraction adapts to the server.</strong>
            <span>Text PDFs and DOCX files are extracted when supported. Scanned images use OCR when available; otherwise NurseLink keeps the file and asks only for the missing information.</span>
          </div>
        ` : ''}
  ...
  function smartInput(field, label, type, profile, suggestions, fieldStatuses, options = {}) {
    const candidate = smartSuggestion(field, profile, suggestions);
    const required = options.required ? 'required' : '';
    const placeholder = options.placeholder ? `placeholder="${smartEsc(options.placeholder)}"` : '';
    const min = options.min !== undefined ? `min="${smartEsc(options.min)}"` : '';
    const max = options.max !== undefined ? `max="${smartEsc(options.max)}"` : '';
    const autocomplete = options.autocomplete ? `autocomplete="${smartEsc(options.autocomplete)}"` : '';
    const value = candidate.value ?? '';
    return `
      <label class="nurselink-smart557-field">
        <span>${smartEsc(label)}${options.required ? ' *' : ''}</span>
        <input name="${smartEsc(field)}" type="${smartEsc(type)}" value="${smartEsc(value)}" ${required} ${placeholder} ${min} ${max} ${autocomplete}>
        ${smartFieldMeta(field, profile, suggestions, fieldStatuses)}
      </label>
    `;
  }

  function smartSelect(field, label, profile, suggestions, fieldStatuses, choices, required = false) {
    const candidate = smartSuggestion(field, profile, suggestions);
    return `
      <label class="nurselink-smart557-field">
        <span>${smartEsc(label)}${required ? ' *' : ''}</span>
        <select name="${smartEsc(field)}" ${required ? 'required' : ''}>
          <option value="">Select</option>
          ${choices.map(([value, text]) => `<option value="${smartEsc(value)}" ${String(candidate.value) === String(value) ? 'selected' : ''}>${smartEsc(text)}</option>`).join('')}
        </select>
        ${smartFieldMeta(field, profile, suggestions, fieldStatuses)}
      </label>
    `;
  ...
  function smartPersonalStep(data) {
    const profile = data?.profile || {};
    const suggestions = data?.suggestions || {};
    const fieldStatuses = data?.field_statuses || {};
    const missing = smartMissingForStep(data?.missing, 2);

    return `
      <section class="nurselink-smart557-card">
        <div class="nurselink-smart557-card-heading">
          <span class="nurselink-smart557-eyebrow">STEP 2 OF 4</span>
          <h1>Confirm Your Personal Information</h1>
          <p>NurseLink has pre-filled anything it could confidently detect. Correct any value before continuing.</p>
          <div class="nurselink-smart561-legend"><span data-status="extracted">Extracted</span><span data-status="applicant_confirmed">Applicant Confirmed</span><span data-status="missing">Missing</span><span data-status="needs_review">Needs Review</span></div>
        </div>

        ${missing.length ? `
          <div class="nurselink-smart557-missing">
            <strong>We still need ${missing.length} item${missing.length === 1 ? '' : 's'} from you.</strong>
            <span>${missing.map(item => smartEsc(item.label)).join(' · ')}</span>
          </div>
        ` : `<div class="nurselink-smart557-complete-callout">✓ Personal information looks complete. Please review it once before saving.</div>`}

        <form class="nurselink-smart557-form" data-smart-form="personal">
          <div class="nurselink-smart557-grid three">
            ${smartInput('first_name', 'First Name', 'text', profile, suggestions, fieldStatuses, { required: true, autocomplete: 'given-name' })}
            ${smartInput('middle_name', 'Middle Name', 'text', profile, suggestions, fieldStatuses, { autocomplete: 'additional-name' })}
            ${smartInput('last_name', 'Last Name', 'text', profile, suggestions, fieldStatuses, { required: true, autocomplete: 'family-name' })}
          </div>
          <div class="nurselink-smart557-grid two">
            ${smartInput('birth_date', 'Date of Birth', 'date', profile, suggestions, fieldStatuses, { required: true, autocomplete: 'bday' })}
            ${smartSelect('sex', 'Sex', profile, suggestions, fieldStatuses, [
              ['female', 'Female'], ['male', 'Male'], ['non_binary', 'Non-binary'], ['prefer_not_to_say', 'Prefer not to say'], ['other', 'Other']
            ])}
          </div>
          <div class="nurselink-smart557-grid two">
            ${smartInput('nationality', 'Nationality', 'text', profile, suggestions, fieldStatuses, { required: true })}
            <label class="nurselink-smart557-field"><span>Email Address</span><input type="email" value="${smartEsc(profile.email || '')}" disabled><small>Confirmed from your NurseLink account</small></label>
          </div>
          <div class="nurselink-smart557-grid two">
            ${smartInput('phone', 'Mobile Number', 'tel', profile, suggestions, fieldStatuses, { required: true, autocomplete: 'tel', placeholder: '+63 9XX XXX XXXX' })}
            ${smartInput('country', 'Country of Residence', 'text', profile, suggestions, fieldStatuses, { required: true, autocomplete: 'country-name' })}
          </div>
          ${smartInput('address_line1', 'Street / Address', 'text', profile, suggestions, fieldStatuses, { autocomplete: 'street-address' })}
          <div class="nurselink-smart557-grid two">
            ${smartInput('city', 'City / Municipality', 'text', profile, suggestions, fieldStatuses, { autocomplete: 'address-level2' })}
            ${smartInput('province', 'Province / State', 'text', profile, suggestions, fieldStatuses, { autocomplete: 'address-level1' })}
          </div>

          <div class="nurselink-smart557-actions split">
            <button type="button" class="secondary-button" data-smart-next="1">← Documents</button>
  ...
  function smartProfessionalStep(data) {
    const profile = data?.profile || {};
    const suggestions = data?.suggestions || {};
    const fieldStatuses = data?.field_statuses || {};
    const missing = smartMissingForStep(data?.missing, 3);

    return `
      <section class="nurselink-smart557-card">
        <div class="nurselink-smart557-card-heading">
          <span class="nurselink-smart557-eyebrow">STEP 3 OF 4</span>
          <h1>Confirm Your Professional Details</h1>
          <p>Review credentials and career information detected from your license, CV, diploma, and certificates.</p>
          <div class="nurselink-smart561-legend"><span data-status="extracted">Extracted</span><span data-status="applicant_confirmed">Applicant Confirmed</span><span data-status="missing">Missing</span><span data-status="needs_review">Needs Review</span></div>
        </div>

        ${missing.length ? `
          <div class="nurselink-smart557-missing">
            <strong>Complete these professional details.</strong>
            <span>${missing.map(item => smartEsc(item.label)).join(' · ')}</span>
          </div>
        ` : `<div class="nurselink-smart557-complete-callout">✓ Required professional information is complete. Please verify it before saving.</div>`}

        <form class="nurselink-smart557-form" data-smart-form="professional">
          <div class="nurselink-smart557-grid two">
            ${smartInput('professional_title', 'Professional Title', 'text', profile, suggestions, fieldStatuses, { required: true, placeholder: 'Registered Nurse' })}
            ${smartInput('years_experience', 'Years of Nursing Experience', 'number', profile, suggestions, fieldStatuses, { required: true, min: 0, max: 80 })}
          </div>
          <div class="nurselink-smart557-grid two">
            ${smartInput('current_position', 'Current / Most Recent Position', 'text', profile, suggestions, fieldStatuses, { placeholder: 'Staff Nurse' })}
            ${smartInput('current_employer', 'Current / Most Recent Employer', 'text', profile, suggestions, fieldStatuses, { placeholder: 'Hospital or facility' })}
          </div>
          ${smartInput('specialty', 'Specialty / Clinical Area', 'text', profile, suggestions, fieldStatuses, { placeholder: 'ICU, ER, Medical-Surgical, etc.' })}
          <div class="nurselink-smart557-grid two">
            ${smartInput('primary_license_number', 'Primary Nursing License / PRC Number', 'text', profile, suggestions, fieldStatuses, { placeholder: 'License or registration number' })}
            ${smartInput('primary_license_country', 'License Country / Jurisdiction', 'text', profile, suggestions, fieldStatuses, { placeholder: 'Philippines, United Kingdom, etc.' })}
          </div>
          <div class="nurselink-smart557-grid two">
            ${smartInput('primary_license_expiry', 'License Expiry Date', 'date', profile, suggestions, fieldStatuses)}
          </div>
          <div class="nurselink-smart557-grid two">
            ${smartInput('highest_nursing_education', 'Highest Nursing Education', 'text', profile, suggestions, fieldStatuses, { required: true, placeholder: 'Bachelor of Science in Nursing' })}
            ${smartInput('graduation_year', 'Graduation Year', 'number', profile, suggestions, fieldStatuses, { min: 1940, max: new Date().getFullYear() + 1 })}
          </div>

          <div class="nurselink-smart557-review-note">
            <strong>Important:</strong> Document extraction does not verify a professional license or credential. Verification remains a separate NurseLink reviewer action.
          </div>

          <div class="nurselink-smart557-actions split">
            <button type="button" class="secondary-button" data-smart-next="2">← Personal</button>
  ...
  function smartReviewRow(label, value, missing = false) {
    return `<div class="nurselink-smart557-review-row ${missing ? 'missing' : ''}"><span>${smartEsc(label)}</span><strong>${smartEsc(value || 'Missing')}</strong></div>`;
  }

  function smartReviewStep(data) {
    const profile = data?.profile || {};
    const documents = Array.isArray(data?.documents) ? data.documents : [];
    ...
    return `
      <section class="nurselink-smart557-card">
        <div class="nurselink-smart557-card-heading">
          <span class="nurselink-smart557-eyebrow">STEP 4 OF 4</span>
          <h1>Review & Submit Your Application</h1>
          <p>Confirm the information below. You remain responsible for correcting any value that was extracted incorrectly.</p>
        </div>

        ${missing.length ? `
          <div class="nurselink-smart557-missing strong">
            <strong>${missing.length} item${missing.length === 1 ? '' : 's'} still need attention.</strong>
            <div>${missing.map(item => `<button type="button" data-smart-next="${item.step}">${smartEsc(item.label)} →</button>`).join('')}</div>
          </div>
        ` : `<div class="nurselink-smart557-complete-callout">✓ Your Smart Registration is complete and ready to submit.</div>`}

        <div class="nurselink-smart557-review-grid">
          <section>
            <div class="nurselink-smart557-review-head"><strong>Personal Information</strong><button type="button" data-smart-next="2">Edit</button></div>
            ${smartReviewRow('Name', [profile.first_name, profile.middle_name, profile.last_name].filter(Boolean).join(' '), !profile.first_name || !profile.last_name)}
            ${smartReviewRow('Date of Birth', profile.birth_date, !profile.birth_date)}
            ${smartReviewRow('Nationality', profile.nationality, !profile.nationality)}
            ${smartReviewRow('Email', profile.email, !profile.email)}
            ${smartReviewRow('Mobile', profile.phone, !profile.phone)}
            ${smartReviewRow('Country', profile.country, !profile.country)}
          </section>
          <section>
            <div class="nurselink-smart557-review-head"><strong>Professional Details</strong><button type="button" data-smart-next="3">Edit</button></div>
            ${smartReviewRow('Professional Title', profile.professional_title, !profile.professional_title)}
            ${smartReviewRow('Experience', profile.years_experience !== null && profile.years_experience !== undefined ? `${profile.years_experience} years` : '', profile.years_experience === null || profile.years_experience === undefined)}
            ${smartReviewRow('Position', profile.current_position || 'Not provided')}
            ${smartReviewRow('Employer', profile.current_employer || 'Not provided')}
            ${smartReviewRow('License / PRC', profile.primary_license_number || 'Document evidence supplied')}
            ${smartReviewRow('License Jurisdiction', profile.primary_license_country || 'Not provided')}
            ${smartReviewRow('Education', profile.highest_nursing_education, !profile.highest_nursing_education)}
          </section>
        </div>

        <div class="nurselink-smart557-review-documents">
          <div class="nurselink-smart557-review-head"><strong>Supporting Documents</strong><button type="button" data-smart-next="1">Manage</button></div>
  ...
  function bindSmartRegistration557(root) {
    if (!root) return;

    const firstBind = root.dataset.smartBound !== '1';
    ...
    if (firstBind) {
      ...
      const replace = event.target.closest('[data-smart-replace-document]');
      if (replace) {
        event.preventDefault();
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = '.pdf,.jpg,.jpeg,.png,.doc,.docx,application/pdf,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document';
        ...
        input.addEventListener('change', async () => {
          ...
          smartRegistration557State.notice = `Replacing document with ${file.name}…`;
          smartRegistration557State.noticeTone = 'info';
          renderSmartRegistration557(document.querySelector('.page') || document.querySelector('main'));
          try {
            const body = new FormData();
            body.append('document', file);
            body.append('replace_document_id', replace.dataset.smartReplaceDocument);
            const result = await smartRegistrationRequest('/documents', { method: 'POST', body });
            smartRegistration557State.notice = result?.message || 'Document replaced and analyzed.';
            smartRegistration557State.noticeTone = 'success';
          } catch (error) {
            smartRegistration557State.notice = error.message;
            smartRegistration557State.noticeTone = 'error';
          }
          await loadSmartRegistration557(true).catch(() => {});
          renderSmartRegistration557(document.querySelector('.page') || document.querySelector('main'));
        }, { once: true });
      ...
      const useValue = event.target.closest('[data-smart-use-field]');
      if (useValue) {
        event.preventDefault();
        const input = root.querySelector(`[name="${CSS.escape(useValue.dataset.smartUseField)}"]`);
        if (input) {
          input.value = useValue.dataset.smartUseValue || '';
          input.dispatchEvent(new Event('change', { bubbles: true }));
          input.focus();
        }
        return;
      ...
      const remove = event.target.closest('[data-smart-remove-document]');
      if (remove) {
        event.preventDefault();
        if (!confirm('Remove this document from your application?')) return;
        remove.disabled = true;
        try {
          await smartRegistrationRequest(`/documents/${remove.dataset.smartRemoveDocument}`, { method: 'DELETE' });
          smartRegistration557State.notice = 'Document removed.';
          smartRegistration557State.noticeTone = 'success';
          await loadSmartRegistration557(true);
          renderSmartRegistration557(document.querySelector('.page') || document.querySelector('main'));
        } catch (error) {
          smartRegistration557State.notice = error.message;
          smartRegistration557State.noticeTone = 'error';
      ...
      const submit = event.target.closest('[data-smart-submit]');
      if (submit) {
        event.preventDefault();
        submit.disabled = true;
        submit.textContent = 'Submitting…';
        try {
          const result = await smartRegistrationRequest('/submit', { method: 'POST', body: JSON.stringify({}) });
          smartRegistration557State.notice = result?.message || 'Application submitted.';
          smartRegistration557State.noticeTone = 'success';
          await loadSmartRegistration557(true);
          renderSmartRegistration557(document.querySelector('.page') || document.querySelector('main'));
        } catch (error) {
    ...
    root.addEventListener('change', async event => {
      const input = event.target.closest('#nurselinkSmart557Files');
      if (!input || !input.files?.length) return;

      const files = [...input.files];
      input.disabled = true;
      smartRegistration557State.notice = `Uploading ${files.length} document${files.length === 1 ? '' : 's'}…`;
      smartRegistration557State.noticeTone = 'info';
      renderSmartRegistration557(document.querySelector('.page') || document.querySelector('main'));
      ...
      for (const file of files) {
        try {
          const body = new FormData();
          body.append('document', file);
          await smartRegistrationRequest('/documents', { method: 'POST', body });
          uploaded++;
        } catch (error) {
      ...
      smartRegistration557State.notice = errors.length
        ? `${uploaded} uploaded. ${errors.join(' ')}`
        : `${uploaded} document${uploaded === 1 ? '' : 's'} uploaded and analyzed.`;
      smartRegistration557State.noticeTone = errors.length ? 'warning' : 'success';
      await loadSmartRegistration557(true);
      renderSmartRegistration557(document.querySelector('.page') || document.querySelector('main'));
    });

    root.addEventListener('submit', async event => {
      const form = event.target.closest('[data-smart-form]');
      if (!form) return;
      event.preventDefault();
      if (!form.reportValidity()) return;
      ...
      const section = form.dataset.smartForm;
      const state = smartRegistration557State.data || {};

      if (section === 'professional') {
        if (data.years_experience !== '') data.years_experience = Number(data.years_experience);
        if (data.graduation_year !== '') data.graduation_year = Number(data.graduation_year);
        else delete data.graduation_year;
      }

      const fields = Object.keys(data);
      data.sources = smartSourcesFor(fields, state.suggestions || {}, data);

      try {
        const result = await smartRegistrationRequest(`/${section}`, {
          method: 'PATCH',
          body: JSON.stringify(data)
        });
        smartRegistration557State.notice = result?.message || 'Information saved.';
        smartRegistration557State.noticeTone = 'success';
        await loadSmartRegistration557(true);
        smartSetStep(section === 'personal' ? 3 : 4);
      } catch (error) {
        smartRegistration557State.notice = error.message;
        smartRegistration557State.noticeTone = 'error';
        if (button) {
    ...
    const drop = root.querySelector('.nurselink-smart557-dropzone');
    if (drop) {
      ...
      drop.addEventListener('drop', event => {
        const input = root.querySelector('#nurselinkSmart557Files');
  ...
  function renderSmartRegistration557(page) {
    ...
    let root = page.querySelector('.nurselink-smart557');
    if (!root) {
      root = document.createElement('section');
      root.className = 'nurselink-smart557';
      const header = page.querySelector('.page-header');
      if (header) header.insertAdjacentElement('afterend', root);
      else page.insertBefore(root, page.firstChild);
    }
    ...
    if (!smartRegistration557State.loaded || !data) {
      root.innerHTML = `
        ${smartStepHeader(step, 0)}
        <div class="nurselink-smart557-card nurselink-smart557-loading">
          <span class="spinner"></span>
          <strong>Preparing your Smart Registration…</strong>
          <p>Loading your saved application and document extraction status.</p>
        </div>
      `;
      bindSmartRegistration557(root);
      loadSmartRegistration557()
        .then(() => renderSmartRegistration557(page))
        .catch(error => {
          smartRegistration557State.notice = error.message;
          smartRegistration557State.noticeTone = 'error';
          root.innerHTML = `${smartStepHeader(step, 0)}${smartNoticeHtml()}<div class="nurselink-smart557-card"><h1>Smart Registration needs attention</h1><p>Unable to load your application right now.</p><button class="primary-button" type="button" onclick="location.reload()">Try Again</button></div>`;
        });
      return;
    }
  ...
  function runEnhancements() {
    ...
    enhanceAdministratorInvitationAcceptance();
  }
