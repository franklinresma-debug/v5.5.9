...
return new class extends Migration
{
    public function up(): void
    {
        $this->recreateEmptyIncompatibleTable(
            ...
            function (): void {
                Schema::create('nurselink_career_preferences', function (Blueprint $table): void {
                    $table->id();
                    $this->userIdDefinition($table);
                    $table->text('desired_roles')->nullable();
                    $table->text('specialties')->nullable();
                    $table->text('target_countries')->nullable();
                    $table->text('work_settings')->nullable();
                    $table->text('employment_types')->nullable();
                    $table->boolean('open_to_overseas')->default(false);
                    $table->boolean('open_to_relocation')->default(false);
                    $table->boolean('open_to_telehealth')->default(false);
                    $table->date('available_from')->nullable();
                    $table->string('preferred_currency', 8)->nullable();
                    $table->decimal('minimum_monthly_compensation', 12, 2)->nullable();
                    $table->string('career_stage', 60)->nullable();
    ...
    {
        Schema::dropIfExists('nurselink_career_preferences');
    }
